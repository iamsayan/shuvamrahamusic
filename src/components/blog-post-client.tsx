...
interface BlogPostClientProps {
  ...
  keepReadingPosts: BlogPost[];
  otherArticlesPosts: BlogPost[];
}
...
export default function BlogPostClient({
  ...
  // Listen for real-time postMessage preview events from Cockpit and map them dynamically
  const post = useLivePreview<Post, BlogPost>(
    initialPost,
    'posts',
    (rawPost) => {
      const entryCopy = { ...rawPost };

      // Fall back to default/skeleton values if the preview payload has empty content fields
      if (!entryCopy.title && initialPost.title) {
        entryCopy.title = initialPost.title;
      }
